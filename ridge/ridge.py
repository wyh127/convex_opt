...
def compute_square_loss(X, y, theta):
    ...
    loss = 1/(2*m) * np.sum(np.square(tmp))
    return loss
...
def regularized_grad_descent(X, y, alpha=0.1, lambda_reg=1, num_iter=1000):
    ...
    #TODO
    theta_hist[0] = theta
    loss_hist[0] = compute_square_loss(X, y, theta) + lambda_reg*np.sum(np.square(theta))
    for i in range(num_iter):
        theta = theta - alpha * compute_regularized_square_loss_gradient(X, y, theta, lambda_reg)
        theta_hist[i+1] = theta
        loss_hist[i+1] = compute_square_loss(X, y, theta) + lambda_reg*np.sum(np.square(theta))
    return theta_hist, loss_hist
    ...
    for i in range(num_iter):
        ...
        for j in shuffle:
            ...
            if isinstance(alpha, str):
                if alpha == "1/sqrt(t)":
                    theta = theta - 1/sqrt(step_size) * compute_regularized_square_loss_gradient(X[j], y, theta, lambda_reg)
                elif alpha == "1/t":
                    theta = theta - 1/step_size * compute_regularized_square_loss_gradient(X[j], y, theta, lambda_reg)
            else:
                theta = theta - alpha * compute_regularized_square_loss_gradient(X[j], y, theta, lambda_reg)
